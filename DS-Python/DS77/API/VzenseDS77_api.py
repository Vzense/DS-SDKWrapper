from DS77.API.VzenseDS77_define import * 
import sys, os
import ctypes
from ctypes import *
gCallbackFuncList=[]
...
class VzenseTofCam():
    ...
    def __init__(self):
        if platform.system() == 'Linux':
            libpath = (os.path.abspath(os.path.dirname(os.getcwd()) + os.path.sep + "../../"))+"/Lib/vzenseDS_api.dll.so"
            print(libpath)
            ...
            libpath = (os.path.abspath(os.path.dirname(os.getcwd()) + os.path.sep + "../../"))+"/Lib/vzenseDS_api.dll"
            print(libpath)
            self.vz_cam_lib = cdll.LoadLibrary(libpath)    
        else:
    ...
    def VZ_ConvertDepthFrameToPointCloudVector(self, depthFrame = VzFrame()): 
        ...
        tmp =VzVector3f*len
        pointlist = tmp()
        return self.vz_cam_lib.VZ_ConvertDepthFrameToPointCloudVector(self.device_handle, depthFrame,pointlist),pointlist
