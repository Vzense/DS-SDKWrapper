...
def pullSDK(url):  
    ...
    if execSysCommand(url) == 'success':
        ...
        if system_config == 'Windows64' or system_config == 'Windows32':
            src = sdkPath + "/DS-BaseSDK/Windows/Bin/x64"
            if system_config != 'Windows64':
                src = sdkPath + "/DS-BaseSDK/Windows/Bin/x86"
            dst = curPath+ "/Lib"
            ...
            src = sdkPath + "/DS-BaseSDK/%s/Lib" %('Ubuntu18.04')
            dst = curPath + "/Lib"
            if os.path.exists(dst):
                shutil.rmtree(dst)
            shutil.move(src, dst)
            ...
            src = sdkPath + "/DS-BaseSDK/%s/Lib" %(system_config)
            dst = curPath + "/Lib"
            if os.path.exists(dst):
                shutil.rmtree(dst)
            shutil.move(src, dst)
    ...
    with open(filename,"r") as fr:
        ...
        if len(lines) == 2 :
            if re.search(r'system',lines[0], re.M|re.I) \
                ...
                if len(tmp_list) == 2:
                    url_config = tmp_list[1].strip()+ '/Vzense/DS-BaseSDK.git'
                return True    
            else:
                return False 
        else:
...
cmdlist = ['git','clone','-b','Ver1.0.6','--depth=1',url_config]
pullSDK(cmdlist)
